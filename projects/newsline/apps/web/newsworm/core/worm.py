...
class WDomItem(DomItem):
	...
	def nested_items(self, ni):
		...
		if isinstance(ni, DomItem): self._nested_items.append(ni)
			...
			try:
				if 'autogen' in ni:
					self._nested_items.append(WDIAutoGen(ni['name'], ni['url'], ni['nested_items'] if 'nested_items' in ni else None, ni['autogen'], ni['range'] if 'range' in ni else None))
				else:
			...
			if helpers.is_empty(ni): raise Exception("You cannot supply nested_items as empty")
				...
				try:
					self._nested_items.extend([(WDIAutoGen(i['name'], i['url'], i['nested_items'] if 'nested_items' in i else None, i['autogen'], i['range'] if 'range' in i else None) if 'autogen' in i else DomItem(i['name'], i['url'], i['selector'], i['nested_items'] if 'nested_items' in i else None )) if isinstance(i, dict) else i for i in ni]) 
				except Exception as e:
					raise Exception("DomItem nested element exception : %s" % str(e))
...
class WDIAutoGen(WDomItem):
	...
	"""
	def __init__(self, name, url, nested_items=None, autogen=False, _range=None):
	...
	def __repr__(self):
		return "WDomItemObject{'name': %s, 'url': %s, 'is_autogen': %s, 'range': %s}"	% (self.name, self.url, self._is_autogenerated, self._range)	

	def __str__(self):
		return "WDomItemObject{'name': %s, 'url': %s, 'is_autogen': %s, 'range': %s}}"	% (self.name, self.url, self._is_autogenerated, self._range)

	def templify(self, link):
		part = self.url.replace(link, '')
		if part == self.url:
			return helpers.templify(link)
		else:
			return link + helpers.templify(part)
	...
	def domitems(self, domitems):
		if helpers.is_list(domitems):
			if not all(helpers.is_dict(domitem) for domitem in domitems):
				...
				for i in domitems:
					if 'autogen' in i:
						self._domitems = WDIAutoGen(i['name'], i['url'], i['nested_items'] if 'nested_items' in i else None, i['autogen'], i['range'] if 'range' in i else None)
					else:
			...
			if 'autogen' in domitems:
				self._domitems = WDIAutoGen(domitems['name'], domitems['url'], domitems['nested_items'] if 'nested_items' in domitems else None, domitems['autogen'], domitems['range'] if 'range' in domitems else None)
			else:
