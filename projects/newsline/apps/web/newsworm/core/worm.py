...
class WDIAutoGen(WDomItem):
	...
	@is_autogenerated.setter
	def is_autogenerated(self, autogen):
		if isinstance(autogen, str):
			if autogen == "True":
...
class Worm(logger.ClassUsesLog):
	...
	@classmethod
	def normalize_summary(cls, summary):
		articles = []
		def niprint(d):
			if "item_type" in d:
