...
class WDIAutoGen(WDomItem):
	...
	"""
	def __init__(self, name, url, domselector, nested_items=None, crawled_items=[], autogenopt=False):
		WDomItem.__init__(self, name, url, domselector, nested_items, crawled_items)
		if autogenopt != False:
			if not isinstance(autogenopt, dict): raise Exception("Autogeneration options must be a dictionary")

			self.crawled_items = self.autogenerate(autogenopt)
